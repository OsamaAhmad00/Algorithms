...
std::vector<int> get_minimum_rotation_amounts(std::string string)
{
    // More efficient solutions:
    // https://en.wikipedia.org/wiki/Lexicographically_minimal_string_rotation

    // This is a left rotation. If you want the amount of the right rotations
    //  subtract each rotation from n.
    ...
    for (int i = 0; i < suffix_array.size(); i++) {
        // We only consider rotations with rotation amount < n
        if (suffix_array[i] < n)
        {
            result.push_back(suffix_array[i]);

            // If there were multiple valid rotations,
            //  they'd be next to each other. Also,
            //  since we've doubled the string, for
            //  every suffix s with rotations < n,
            //  s.size() >= n.
            // Note is that the bigger the rotation
            //  (suffix starting index) is, the smaller
            //  the size of the suffix.
            //  Because of this fact, bigger rotations
            //  will be previous to the smaller ones.
            // If you want only the biggest rotation, return
            //  only the first valid rotation encounter (no
            //  need for the loop below). If you want only
            //  the smallest rotation, return the last valid
            //  rotation you encounter.
            // Because of the fact above, the returned rotations
            //  will be sorted in descending order.
            while (i < LCP_array.size() && LCP_array[i] >= n)
                result.push_back(suffix_array[++i]);
...
int get_minimum_rotation_amount(std::string string)
{
    int n = string.size();
...
std::string get_minimum_rotation(const std::string& string)
{
    int amount = get_minimum_rotation_amount(string);
    return string.substr(amount, string.size() - amount) + string.substr(0, amount);
}

void test(const std::string& string)
{
    std::cout << "String: " << string << std::endl;
    std::cout << "Lexicographically minimum rotation: " << get_minimum_rotation(string) << std::endl;
    std::cout << "Minimum rotation amounts: ";
    for (int amount : get_minimum_rotation_amounts(string)) {
        std::cout << amount << ' ';
    }
