...
int get_unique_substrings_count(const std::vector<int>& LCP)
{
    // string size.
    int n = LCP.size() + 1;

    int result = (n * (n + 1)) / 2;  // All possible substrings.
    for (int lcp : LCP)
        result -= lcp;  // Removing the repeated strings.
...
void add_substrings(std::vector<SubstringInfo>& info, int start_index, int min_length, int max_length)
{
    for (int l = min_length; l <= max_length; l++) {
        info.push_back({start_index, l});
    }
}

std::vector<SubstringInfo> get_unique_substrings_info( const std::vector<int>& LCP_array,
                                                       const std::vector<int>& suffix_array )
{
    // https://www.youtube.com/watch?v=m2lZRmMjebw
    // As a result of the suffix array being sorted, the
    // returned substrings are sorted as well.

    std::vector<SubstringInfo> result;

    int n = LCP_array.size() + 1;

    for (int i = 0; i < LCP_array.size(); i++)
    {
        int lcp = LCP_array[i];
        int start_index = suffix_array[i];
        int suffix_length = n - start_index;

        add_substrings(result, start_index, lcp + 1, suffix_length);
    }

    // We can get rid of this line if the LCP array has an additional 0 at the end.
    add_substrings(result, suffix_array[n-1], 1, n - suffix_array[n - 1]);

    return result;
}

std::vector<std::string> get_unique_substrings( const std::vector<int>& LCP_array,
                                                const std::vector<int>& suffix_array,
                                                const std::string& string )
{
    std::vector<std::string> result;

    auto info = get_unique_substrings_info(LCP_array, suffix_array);

    for (auto& i : info) {
        result.push_back(string.substr(i.start_index, i.length));
    }
...
void test(const std::string& string)
{
    ...
    auto substrings =  get_unique_substrings(LCP_array, suffix_array, string);

    std::cout << "String: " << string << std::endl;
    std::cout << "Substrings Count: " << get_unique_substrings_count(LCP_array) << std::endl;
