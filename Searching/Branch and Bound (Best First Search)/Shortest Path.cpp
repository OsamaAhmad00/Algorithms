...
public:
    ...
    int compute_weight(int from, int to)
    {
         ...
         *
         * THIS FUNCTION ASSUMES THAT THE SOURCE AND THE TARGET ARE
         *  CONNECTED. THIS WILL GO INTO AN INFINITE LOOP IN CASE THERE
        ...
        while (!queue.empty())
        {
            ...
            for (const Edge& e : graph[current.node])
            {
                ...
                *
                * This way is slow because we keep extending nodes
                *  that are already been extended. Also, always extending
                *  until we reach the target will cause an infinite loop
                *  if the source and the target are not connected. It makes
                *  sense to discard nodes that are been extended before.
                */
